...
Wait for confirmation before downloading or changing anything. Do not decide the security-process question yourself. Record the answer as `SECURITY_FLOW` (yes/no) - it determines whether Phase 7 runs after the upgrade.
...
## PHASE 7: Security Process (only if `SECURITY_FLOW` = yes)

Skip this phase entirely for a routine upgrade. If Step 3.2 established that this upgrade addresses a security issue and the user chose to follow Moodle's security process, continue here immediately after Phase 5 (and Phase 6, if this library has custom patches) finishes for the main/current branch.

### Step 7.1 - Confirm Proceeding with the Security Flow
...
### Step 7.2 - Determine Affected Moodle Versions
...
### Step 7.3 - Update the Jira Issue
...
### Step 7.4 - Full Upgrade for Main

This is Phase 4 + Phase 5 + Phase 6 (if applicable), applied to the `main`/current branch - complete it now if it hasn't already run in this session. Only `main` gets the full library upgrade; affected stable branches get a fix-only patch instead (Step 7.7), not a version bump.

### Step 7.5 - Push the Main Patch
...
### Step 7.6 - Get the Root Dir for the Next Affected Version

For each Moodle version confirmed as affected in Step 7.2 that hasn't been patched yet, ask:
> "What's the root directory of your [version] checkout?"

### Step 7.7 - Apply the Security Fix Only

In that checkout, cherry-pick the upstream security-fix commit(s) if they apply cleanly, or manually apply just the security fix - do not upgrade the whole library on this branch; it stays on its existing library version otherwise. Show the diff before writing anything, same as Steps 4.2/6.1.

If the fix touches anything under `amd/src`, run `nvm use` followed by `grunt amd` in that checkout before committing, same as Step 4.3.

### Step 7.8 - Commit and Push the Patch
...
Go back to Step 7.6 for the next affected version. Repeat until every version listed in Step 7.2 has a patch pushed. Once none remain, summarize all branches patched and stop.
